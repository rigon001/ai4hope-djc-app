...
def initialize_database():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS interactions (
            ...
            answer_rating TEXT,
            source_ratings TEXT,
            extended_clicked INTEGER DEFAULT 0,
            timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY(user_id) REFERENCES users(user_id)
        );
    """)
    
    # Migration: Add extended_clicked column to interactions if it doesn't exist
    try:
        cursor.execute("ALTER TABLE interactions ADD COLUMN extended_clicked INTEGER DEFAULT 0")
        conn.commit()
    except sqlite3.OperationalError:
        # Column already exists, no migration needed
        pass
...
def add_interaction(user_id, question, answer_style, answer, answer_rating, source_ratings, extended_clicked=False):
    """Insert a new user interaction record."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        """
        INSERT INTO interactions (user_id, question, answer_style, answer, answer_rating, source_ratings, extended_clicked)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (user_id, question, answer_style, answer, answer_rating, json.dumps(source_ratings), int(extended_clicked))
    )
    conn.commit()
    conn.close()
